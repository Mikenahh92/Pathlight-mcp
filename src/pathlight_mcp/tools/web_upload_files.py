...
:func:`~pathlight_mcp.tools.web_connect`.  The tool locates the file input
...
explicit user opt-in (``SYSTEM_ACTION_RISK_MAP`` in :mod:`pathlight_mcp.safety`).

Tool-layer only — no ABC changes.  Relies on the existing
:class:`~pathlight_mcp.backends.web.WebBackend` for session management,
:class:`~pathlight_mcp.cdp.domains.dom.DOMDomain` for element lookup and
...
def _find_backend_node_id(
    ...
    from pathlight_mcp.backends.router import TaggedHandle
