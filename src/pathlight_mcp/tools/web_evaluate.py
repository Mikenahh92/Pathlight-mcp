...
established via :func:`~pathlight_mcp.tools.web_connect` — the window reference
...
2. Sliding-window rate limiting via :class:`~pathlight_mcp.safety.EvaluateRateLimiter`
3. Execution timeout passed to ``Runtime.evaluate``
4. Result sanitization via :func:`~pathlight_mcp.privacy.redact_web_content`
...
:class:`~pathlight_mcp.cdp.domains.runtime.RuntimeDomain` for evaluation.
"""
...
from pathlight_mcp.privacy import redact_web_content
from pathlight_mcp.safety import EvaluateRateLimiter, classify_system_action
...
def _sanitize_result(value: object) -> object:
    ...
    Applies :func:`~pathlight_mcp.privacy.redact_web_content` to string results
