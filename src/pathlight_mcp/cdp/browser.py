...
HTTP ``/json/list`` endpoint, creates :class:`~pathlight_mcp.cdp.session.CDPSession`
...
class CDPBrowser:
    ...
    sessions.  Wraps a root :class:`~pathlight_mcp.cdp.connection.CDPConnection`
    ...
    Attributes:
        ...
        state: Current :class:`~pathlight_mcp.cdp._types.ConnectionState`.
    """
    ...
    def connection(self) -> CDPConnection | None:
        """The underlying root :class:`~pathlight_mcp.cdp.connection.CDPConnection`."""
        return self._connection

    def connect(self) -> None:
        """Open the browser connection.

        Creates a root :class:`~pathlight_mcp.cdp.connection.CDPConnection` to
    ...
    def list_targets(self, *, target_type: str | None = None) -> list[CDPTarget]:
        ...
        Returns:
            List of discovered :class:`~pathlight_mcp.cdp._types.CDPTarget`
    ...
    def get_target(self, target_id: str) -> CDPTarget | None:
        ...
        Returns:
            The matching :class:`~pathlight_mcp.cdp._types.CDPTarget`, or ``None``.
        """
    ...
    def attach(self, target: CDPTarget) -> CDPSession:
        ...
        Raises:
            PathlightMCPError: If the browser connection is not open.
        """
        if self._state != ConnectionState.CONNECTED or self._connection is None:
            raise PathlightMCPError("Browser connection is not open")
    ...
    def detach(self, target_id: str) -> None:
        ...
        Raises:
            PathlightMCPError: If no session exists for the target.
        """
        with self._lock:
        ...
        if session is None:
            raise PathlightMCPError(f"No active session for target {target_id}")
    ...
    def get_session(self, target_id: str) -> CDPSession | None:
        ...
        Returns:
            The :class:`~pathlight_mcp.cdp.session.CDPSession`, or ``None``.
        """
        with self._lock:
    ...
    def send_command(
        ...
        *,
        ...
        For target-scoped commands, use a :class:`~pathlight_mcp.cdp.session.CDPSession`.
    ...
    def _start_event_listener(self) -> None:
        ...
        Watches the connection's :class:`~pathlight_mcp.cdp.events.EventBuffer`
