...
responses, and dispatches events to an :class:`~pathlight_mcp.cdp.events.EventBuffer`.
...
class CDPConnection:
    ...
    :class:`~pathlight_mcp.cdp.events.EventBuffer`.
    ...
    Attributes:
        events: The :class:`~pathlight_mcp.cdp.events.EventBuffer` for received events.
    ...
    def send_command(
        ...
        *,
        ...
        Raises:
            PathlightMCPError: If the browser returns an error response.
    ...
    @staticmethod
    def _map_cdp_error(exc: "Exception") -> PathlightMCPError:
        """Map a CDP protocol error to the appropriate PathlightMCPError.

        Args:
            exc: The raw CDP error.

        Returns:
            A :class:`~pathlight_mcp.errors.PathlightMCPError` subclass.
        """
        from pathlight_mcp.cdp.protocol import CDPError
        ...
        if isinstance(exc, CDPError):
            ...
            return PathlightMCPError(f"CDP error {code}: {message}")

        return PathlightMCPError(str(exc))
