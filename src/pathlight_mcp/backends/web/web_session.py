...
:class:`~pathlight_mcp.backends.web.WebBackend` into a dedicated registry so
...
The registry:
    ...
    - Lazily attaches sessions on first access via
      :meth:`~pathlight_mcp.cdp.browser.CDPBrowser.attach`.
...
from pathlight_mcp.errors import BackendUnavailableError, WindowNotFoundError
...
class WebSessionRegistry:
    """Manages CDP session lifecycle for a :class:`~pathlight_mcp.cdp.browser.CDPBrowser`.

    Args:
        browser_getter: A callable that returns the current
            :class:`~pathlight_mcp.cdp.browser.CDPBrowser` instance.  Using a
    ...
    def get_active(self) -> CDPSession:
        ...
        Returns:
            The first active :class:`~pathlight_mcp.cdp.session.CDPSession`.
    ...
    def put_session(self, key: str, session: CDPSession) -> None:
        ...
        Args:
            ...
            session: The :class:`~pathlight_mcp.cdp.session.CDPSession` to cache.
        """
